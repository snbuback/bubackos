...
docker-build:
	...
	docker run --rm -v $(PWD):$(PWD) bubackos:latest bash -c 'rm -rf $(SYSROOT) && \
		mkdir -p $(SYSROOT)/gcc && cp -av /usr/local/lib/gcc/x86_64-elf/$(GCC_VERSION)/include $(SYSROOT)/gcc'
