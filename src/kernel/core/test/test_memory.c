...
extern memory_region_t* data_mem_region;
...
void test_memory_area_is_resized()
{
    ...
    const size_t mem_region_size = data_mem_region->allocated_size;
    ...
    log_info("size=%d allocated=%d", data_mem_region->size, data_mem_region->allocated_size);
    TEST_ASSERT_TRUE(data_mem_region->allocated_size > mem_region_size);
